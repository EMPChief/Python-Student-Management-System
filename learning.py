...
class AgeCalculator(QWidget):
    ...
    def calculate_age(self):
        ...
        """
        try:
            birth_date = datetime.strptime(self.date_birth_line_edit.text(), "%d/%m/%Y")
            ...
            if years == 0:
                ...
                if months == 0:
                    ...
                    self.output_label.setText(f"{self.name_line_edit.text()} is {days} days old.")
                else:
                    self.output_label.setText(f"{self.name_line_edit.text()} is {months} months old.")
            else:
                self.output_label.setText(f"{self.name_line_edit.text()} is {years} years old.")
        except ValueError:
            self.output_label.setText("Please enter a valid date (dd/mm/yyyy).")
